...
TEST_FILE = test.pl
TEST_FILES = t/01app.t t/02pod.t t/03podcoverage.t t/06.aggregate.controller.t t/07.aggregate.schematests.t t/mime.t t/model_Cleavages.t t/model_FS-Cleavage.t t/view_TT.t
TESTDB_SW = -d
...
ppd :
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Catalyst-Controller-Validation-DFV" VERSION="0,0,2,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-MimeInfo-Magic" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Test-Aggregate" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Test-DBIx-Class-Schema" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
